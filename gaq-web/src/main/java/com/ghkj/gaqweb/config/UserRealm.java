...
public class UserRealm extends AuthorizingRealm {
    ...
    @Autowired
    public UserRealm(UserMapper userMapper, AdminRoleMapper roleMapper, AdminPermissionMapper permissionMapper) {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        // 查询用户角色
        List<AdminRole> roleDtos = roleMapper.findRoleByUser(userDto);
        for (AdminRole roleDto : roleDtos) {
            if (roleDto != null) {
                // 添加角色
                simpleAuthorizationInfo.addRole(roleDto.getRoleName());
                // 根据用户角色查询权限
                List<AdminPermission> permissionDtos = permissionMapper.findPermissionByRole(roleDto);
                for (AdminPermission permissionDto : permissionDtos) {
                    if (permissionDto != null) {
                        // 添加权限
                        simpleAuthorizationInfo.addStringPermission(permissionDto.getPermissionName());
                    }
                }
            }
        }
